...
if (isset($_FILES['statement_document']) && $_FILES['statement_document']['error'] == 0) {
    $target_dir = "uploads/bank_statements/"; // Ensure this directory exists and is writable
    ...
    $unique_filename = $student_id . '_bankstatement_' . $statement_type . '_' . uniqid() . '.' . $file_extension;
    $target_file = $target_dir . $unique_filename;
    
    // Move uploaded file
    if (move_uploaded_file($_FILES['statement_document']['tmp_name'], $target_file)) {
        // Prepare SQL to insert file info
        $stmt = $conn->prepare("INSERT INTO bank_statements (student_id, statement_type, file_name, file_path, notes, uploaded_at) VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("issss", $student_id, $statement_type, $_FILES['statement_document']['name'], $target_file, $notes);
        
        if ($stmt->execute()) {
            header("Location: bank_statement.php?student_id=$student_id&success=Statement uploaded successfully");
        } else {
        ...
        header("Location: bank_statement.php?student_id=$student_id&error=File upload failed");
    }
} else {
    ...
    header("Location: bank_statement.php?student_id=$student_id&error=" . urlencode($error_message));
}
